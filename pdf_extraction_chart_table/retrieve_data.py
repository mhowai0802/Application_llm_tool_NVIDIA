...
import json
from pdf_extraction_chart_table.config import MONGO_CONFIG
...
def main():
    # Get MongoDB credentials from environment variables or input
    ...
    try:
        document_name = input("Enter document name to extract charts from: ")
        extract_all_charts(db, document_name)
    finally:
...
# For import and programmatic usage
def get_charts(document_name, output_dir=None):
    """Function to be imported and used programmatically"""
    ...
    try:
        ...
        result = extract_all_charts(db, document_name, output_dir)
        client.close()
        return result
    except Exception as e:
