...
def display_original_chart(base64_str, title=None):
    try:
        image_bytes = base64.b64decode(base64_str)
        image = Image.open(BytesIO(image_bytes))

        if title:
            st.subheader(f"Original {title} Chart")
...
with col2:
    ...
    for message in st.session_state.messages:
        ...
        elif message["type"] == "chart_image":
            st.markdown(f"File source: {message["from"]}")
    ...
    if prompt_input:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if tool_used == "Chart Retreiver":
                    for chart in final_response['charts']:
                        st.session_state.messages.append({
                            "role": "assistant",
                            "from": final_response['document_name'],
                            "title": chart,
                            "content": final_response['charts'][chart]['image_data'],
                            "type": "chart_image"
                        })
                else:
